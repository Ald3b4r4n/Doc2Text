//Adiciona a configuração do Firebase também no app.js
const firebaseConfig = {
  apiKey: "AIzaSyA-NWBdLBUWErBgqhRFBrfXtHDZhTt7qLA", // usar variavel de ambiente
...
fbAuth.onAuthStateChanged(user => {
  if (user) {
    ...
    if (userInfo) {
      userInfo.textContent = user.email; // Mostra o email do usuário
    }
  } else {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const elements = {
    ...
    correctionModal: new bootstrap.Modal(document.getElementById('correctionModal')),
  ...
  const requiredFields = [
    'abordado', 'genitora', 'cpf', 'dn', 
    'antecedentes', 'endereco', 'local', 'equipe'
  ];
  ...
  elements.cameraBtn.addEventListener('click', () => openFileSelector(elements.fileInput, 'environment'));
  elements.galleryBtn.addEventListener('click', () => openFileSelector(elements.fileInput));
  elements.fileInput.addEventListener('change', handleFileSelect);
  elements.processBtn.addEventListener('click', processDocument);

  elements.cameraBtnAbordado.addEventListener('click', () => openFileSelector(elements.fileInputAbordado, 'environment'));
  elements.galleryBtnAbordado.addEventListener('click', () => openFileSelector(elements.fileInputAbordado));
  elements.fileInputAbordado.addEventListener('change', handleFileSelectAbordado);

  elements.copyBtn.addEventListener('click', copyToClipboard);
  elements.whatsappBtn.addEventListener('click', sendToWhatsApp);
  
  elements.resetAntecedentes.addEventListener('click', () => clearField('antecedentes'));
  ...
  function setupCheckboxPair(checkId, naoAplicaId, conditionalElementId, isInput, onCheckCallback = null) {
  ...
  const extractAndFillPlate = () => {
    if (ocrRawText) {
      ...
      if (match && match[1]) {
        elements.veiculoPlaca.value = match[1].toUpperCase().trim().replace(' ', '-');
      }
    }
  };

  setupCheckboxPair('inserirApelido', 'naoAplicaApelido', 'apelido', true);
  setupCheckboxPair('tornozeleiraCheck', 'naoAplicaTornozeleira', 'tornozeleiraField', false);
  setupCheckboxPair('veiculoCheck', 'naoAplicaVeiculo', 'veiculoFields', false, extractAndFillPlate);
  ...
  function validateFormAndToggleActions() {
    const isFormValid = requiredFields.every(id => elements[id].value.trim() !== '');
  ...
  function handleFileSelect(e) {
    ...
    reader.onload = function (ev) {
      ...
      showAlert(`Imagem selecionada: ${(file.size / 1024 / 1024).toFixed(2)}MB. Clique em "Processar".`, 'info');
    };
  ...
  function handleFileSelectAbordado(e) {
    ...
    reader.onerror = () => showAlert("Erro ao carregar imagem do abordado.", 'danger');
    reader.readAsDataURL(file);
  }
  ...
  async function processDocument() {
    ...
    try {
      ...
      document.querySelectorAll('.edit-btn').forEach(el => el.style.visibility = 'visible');
  ...
  async function callOcrSpaceApi(base64Image) {
    return new Promise(async (resolve, reject) => {
      const timeoutId = setTimeout(() => reject(new Error("Tempo excedido no OCR")), OCR_TIMEOUT);
      ...
      try {
        ...
        if (data.IsErroredOnProcessing) throw new Error(data.ErrorMessage[0] || "Erro na API de OCR");
        if (!data.ParsedResults || data.ParsedResults.length === 0) throw new Error("Nenhum texto foi reconhecido");
  ...
  async function processImageFile(file) {
    ...
    try {
      ...
      const compressedImage = await compressToFileSize(resizedImage, MAX_FILE_SIZE_KB);
  ...
  function extractAndFillFields(text) {
    ...
    elements.abordado.value = findValue(/(?:nome|portador)[\s:]*([A-ZÀ-Ü\s]{8,})/i);
    elements.genitora.value = findValue(/(?:mãe|genitora)[\s:]*([A-ZÀ-Ü\s]{8,})/i);
    elements.dn.value = findValue(/(?:nasc|nascimento)[\s:.]*(\d{2}\/\d{2}\/\d{4})/i);
  ...
  function generateReportText() {
    ...
    if (elements.tornozeleiraCheck.checked) {
      report += `- Possui tornozeleira. Número: ${getVal('tornozeleiraNumero')}\n`;
    } else {
  ...
  async function copyToClipboard() {
    ...
    try {
      ...
      showAlert("Erro ao copiar. Seu navegador pode não suportar esta função.", 'danger');
    }
  }
    ...
    if (navigator.share && abordadoFile && window.isSecureContext) {
      try {
        ...
        if (error.name !== 'AbortError') {
          ...
          showAlert('Não foi possível compartilhar a imagem. Enviando apenas o texto.', 'warning');
      ...
      if (!window.isSecureContext && abordadoFile) {
        alert("ERRO DE SEGURANÇA: O compartilhamento de arquivos só funciona em conexão segura (HTTPS).");
      }
      const encodedText = encodeURIComponent(text);
      window.open(`https://wa.me/?text=${encodedText}`, '_blank');
    }
  ...
  function openCorrectionModal(fieldId) {
    ...
    elements.modalFieldName.textContent = document.querySelector(`label[for="${fieldId}"]`)?.textContent || fieldId;
    elements.ocrTextContent.textContent = ocrRawText || "Nenhum texto OCR para exibir.";
    ['startExpression', 'endExpression', 'resultPreview', 'manualInput'].forEach(id => elements[id].value = '');
  ...
  function fileToBase64(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = (error) => reject(new Error("Falha ao converter arquivo para Base64: " + error));
      reader.readAsDataURL(file);
    });
  }
  ...
  async function resizeImage(file, maxWidth, maxHeight) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob(blob => {
          if (!blob) return reject(new Error("Falha ao criar blob da imagem redimensionada"));
